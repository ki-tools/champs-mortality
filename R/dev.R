...
build_table_location <- function(
    ...
    dplyr::left_join(
        ...
        dat %>%
            ...
            dplyr::rename_if(is.numeric,
                .funs = function(x) {paste0(x, "_percent")}),
    ...
    while (length(while_factors) > 0) {
        ...
        factor_info <- paste0("__", factor_title, "__ _P-value: ",
            pvalue_str, "_ _, Missing (%): ", percent_str, "_")
        dat_gt <- dat_gt %>%
            gt::tab_row_group(
                label = gt::md(factor_info), rows = combine_rows)
    ...
    while (length(gt_while_columns) > 0) {
        ...
        column_title <- paste0(create_name, "<br>N = ",
    ...
    dat_gt %>%
        ...
        gt::tab_source_note(
            source_note = gt::md("Built using the champsmortality R package.")) %>% # nolint
